...
class BatchPreparer:
    ...
    def prepare_trip_batch(batch_meta, device):
        ...
        """
        batch_trip, valid_len, trip_ids = batch_meta
    ...
    # def prepare_trip_batch(batch_meta, device):
    #     batch_trip, valid_len, trip_ids = batch_meta  # 支持 trip_ids
    #     batch_trip = torch.from_numpy(batch_trip).to(device).float()
    #     valid_len = torch.from_numpy(valid_len).to(device).long()
    #     return [batch_trip, valid_len]
