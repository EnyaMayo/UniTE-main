...
2. 生成对比学习的索引数据, loss不是对比学习的话不用这一步              
  ...
  作用：生成并保存对比学习/三元组损失用的索引文件（如ksegsimidx-100-200_0.npz等）。    
  具体内容：包含三元组索引（qry_idx, tgt_idx, neg_idx），用于采样anchor/positive/negative轨迹对。    
  用途：TripletLoss等对比损失训练时必须用到，决定哪些轨迹是正对、负对。    
...
/home/pshao8/poi/data_preprocess.ipynb      
