...
class TripletLoss(nn.Module): # shaoyun

    """
    说明：
    新增 TripletLoss：
    使用 nn.TripletMarginLoss，优化嵌入空间。
    加载 ksegsimidx-100-200，获取 [qry_idx, tgt_idx, neg_idx]。
    输入 metas 为 (trip, lengths)，trip 包含 [tod, road, road_prop, lng, lat, weekday, seq_i, seconds, offset]。
    输出标量损失。
    未包含其他损失：仅实现 TripletLoss，减少复杂性。
    
    更新 TripletLoss 加载三元组索引。
    """
    def __init__(self, data, margin=1.0, triplet_indices='ksegsimidx-100-200', latent_size=128):
        super().__init__()
        self.name = f'triplet-margin{margin}-latent{latent_size}'
        self.data = data
        self.margin = margin
    ...
    def forward(self, models, metas, *args):
        encoder = models[0]
        qry_idx, tgt_idx, neg_idx = self.data.load_meta(self.triplet_indices, 0)
        trip, lengths = metas
        batch_size = trip.shape[0]
        # 检查输入
        with open('debug_log.txt', 'a') as f:
            f.write(f"trip shape: {trip.shape}\n")
            f.write(f"lengths: {lengths.tolist()}\n")
            f.write(f"trip has nan: {torch.isnan(trip).any().item()}\n")
            f.write(f"trip has inf: {torch.isinf(trip).any().item()}\n")
        trip = torch.nan_to_num(trip, nan=0.0)
        embeds = encoder(trip, lengths)
        # 检查 embeds
        with open('debug_log.txt', 'a') as f:
            f.write(f"embeds.shape: {embeds.shape}\n")
            f.write(f"embeds has nan: {torch.isnan(embeds).any().item()}\n")
            f.write(f"embeds has inf: {torch.isinf(embeds).any().item()}\n")
            f.write(f"embeds min: {embeds.min().item()}\n")
            f.write(f"embeds max: {embeds.max().item()}\n")
        num_triplets = min(len(qry_idx), batch_size * 10)
        if num_triplets == 0:
            with open('debug_log.txt', 'a') as f:
                f.write("Warning: No triplets available\n")
            return torch.tensor(0.0, device=trip.device, requires_grad=True)
        sample_idx = torch.randperm(len(qry_idx))[:num_triplets]
        qry_idx = torch.tensor(qry_idx[sample_idx], device=trip.device) % batch_size
        tgt_idx = torch.tensor(tgt_idx[sample_idx], device=trip.device) % batch_size
        neg_idx = torch.tensor(neg_idx[sample_idx], device=trip.device) % batch_size
        with open('debug_log.txt', 'a') as f:
            f.write(f"qry_idx max: {qry_idx.max().item()}\n")
            f.write(f"tgt_idx max: {tgt_idx.max().item()}\n")
            f.write(f"neg_idx max: {neg_idx.max().item()}\n")
        anchor = embeds[qry_idx]
        positive = embeds[tgt_idx]
        negative = embeds[neg_idx]
        loss = self.triplet_loss(anchor, positive, negative)
        with open('debug_log.txt', 'a') as f:
            f.write(f"loss: {loss.item()}\n")
            f.write(f"loss has nan: {torch.isnan(loss).any().item()}\n")
        return loss