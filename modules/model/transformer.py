...
class MLMTransformer(Encoder):
    ...
    def forward(self, trip, valid_len, pretrain=False):
        ...
        con_x = trip[..., self.con_feats]
        assert not torch.isnan(con_x).any(), "con_feats has NaN"
        assert not torch.isinf(con_x).any(), "con_feats has Inf"
        assert con_x.shape[-1] == len(self.con_feats), f"con_x shape: {con_x.shape}, expected last dim {len(self.con_feats)}"
        h += self.con_linear(con_x)
    ...
    def forward(self, trip, valid_len, **kwargs):
        ...
        if self.dis_embeds is not None:
            for dis_embed, dis_feat in zip(self.dis_embeds, self.dis_feats):
                feat = trip[..., dis_feat]
                assert torch.isfinite(feat).all(), f"dis_feat {dis_feat} has NaN/Inf"
                assert (feat >= 0).all(), f"dis_feat {dis_feat} has negative values"
                assert (feat < dis_embed.num_embeddings).all(), f"dis_feat {dis_feat} out of range: max={feat.max()}, num_embeddings={dis_embed.num_embeddings}"
                h += dis_embed(feat.round().long())
        if self.con_linear is not None:
            con_x = x[..., self.con_feats]
            assert not torch.isnan(con_x).any(), "con_feats has NaN"
            assert not torch.isinf(con_x).any(), "con_feats has Inf"
            assert con_x.shape[-1] == len(self.con_feats), f"con_x shape: {con_x.shape}, expected last dim {len(self.con_feats)}"
            h += self.con_linear(con_x)
