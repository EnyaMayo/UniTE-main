...
def preprocess_nyc(path: bytes, preprocessed_path: bytes) -> pd.DataFrame:
    ...
    df['UTCTimeOffsetEpoch'] = df['UTCTimeOffset'].apply(
    ...
    df['UTCTimeOffsetDay'] = df['UTCTimeOffset'].apply(
    ...
    # id encoding
    df['check_ins_id'] = df['UTCTimeOffset'].rank(
        ascending=True, method='first') - 1
    traj_id_le, padding_traj_id = id_encode(
        df, df, 'pseudo_session_trajectory_id')

    # fit encoder on training data, and transform all data
    # new: a better alternative is to use sine-cosine encoding for hour and weekday
    df_train = df[df['SplitTag'] == 'train']
    poi_id_le, padding_poi_id = id_encode(df_train, df, 'PoiId')
    poi_category_le, padding_poi_category = id_encode(
        df_train, df, 'PoiCategoryId')
    ...
    weekday_id_le, padding_weekday_id = id_encode(
        df_train, df, 'UTCTimeOffsetWeekday')
    ...
    # new: maybe ignore the first for all
    df = ignore_first(df)
...
def preprocess_tky_ca(cfg: Cfg, path: bytes) -> pd.DataFrame:
    ...
    data = FileReader.do_filter(
        data, cfg.dataset_args.min_poi_freq, cfg.dataset_args.min_user_freq)
...
def preprocess(cfg: Cfg):
    ...
    validate_file = osp.join(
        preprocessed_path, 'validate_sample_with_traj.csv')
    ...
    preprocessed_result['train_sample'][keep_cols].to_csv(
        train_file, index=False)
    preprocessed_result['validate_sample'][keep_cols].to_csv(
        validate_file, index=False)
    preprocessed_result['test_sample'][keep_cols].to_csv(
