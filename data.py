...
class Data:
    def __init__(self, name, road_type='road_network'):
        ...
        #                    f'{select_set}.npz'
        # )
        self.meta_dir = os.path.join('cache', 'meta', name)
        self.get_meta_path = lambda meta_type, select_set: os.path.join(
            self.meta_dir,
            f'{meta_type}' + ('_grid' if self.road_type == 'grid' else '') + \
            ('' if select_set is None else f'_{select_set}') + '.npz'
        )
    ...
    def read_hdf(self):
        ...
        # 2. 再归一化所有连续特征（最后一步！）
        lng_min, lng_max = self.trips['lng'].min(), self.trips['lng'].max()
        lat_min, lat_max = self.trips['lat'].min(), self.trips['lat'].max()
        self.trips['lng'] = (self.trips['lng'] - lng_min) / (lng_max - lng_min)
        self.trips['lat'] = (self.trips['lat'] - lat_min) / (lat_max - lat_min)
        self.trips['tod'] = self.trips['tod'] % 1.0  # tod 已经是 [0,1]
        self.trips['seconds'] = self.trips['seconds'] / (24*60*60)  # 归一化到 [0,1]
        ...
        print(self.trips[['road', 'road_prop', 'weekday', 'seq_i']].head())
        print(self.trips['road'].unique())
        print(self.trips['road_prop'].unique())
        print(self.trips['weekday'].unique())
        print(self.trips['seq_i'].unique())
    ...
    def load_meta(self, meta_type, select_set):
        ...
        if not hasattr(self, f'loaded_{meta_type}_{select_set}'):
            print('Loaded meta from', meta_path)
            print(f"keys: {list(loaded.keys())}")
            setattr(self, f'loaded_{meta_type}_{select_set}', True)
        if meta_type == 'trip':
            try:
                trips = loaded['trips']
                lengths = loaded['lengths']
                trip_ids = loaded.get('trip_ids', None)
            except KeyError:
                print("Falling back to arr_0, arr_1")
                trips = loaded['arr_0']
                lengths = loaded['arr_1']
                trip_ids = None
            return trips, lengths, trip_ids
            ...
            try:
                ...
                print("Falling back to arr_0, arr_1, arr_2")
                qry_idx = loaded['arr_0']
                tgt_idx = loaded['arr_1']
                neg_idx = loaded['arr_2']
            return qry_idx, tgt_idx, neg_idx
        else:
    ...
    def dump_meta(self, meta_type, select_set):
        ...
        trips = self.trips
        if not trips.empty:
            max_len = trips.groupby('trip').size().max()
            max_trip_len = max(max_trip_len, max_len)
        print(f"Global max_trip_len: {max_trip_len}")
        ...
        trip_normalizer = Normalizer(self.stat, feat_cols=[0, 3, 4, 7], norm_type='minmax')
        ...
        if meta_type == 'trip':
            ...
            trip_ids = np.array(trip_ids, dtype=str)
            print(f"Before normalize - arrs shape: {arrs.shape}")
            print(f"arrs has nan: {np.isnan(arrs).any()}")
            arrs = trip_normalizer(arrs)
            arrs = np.nan_to_num(arrs, nan=0.0)
            # === 这里加上强制类型转换 ===
            for idx in [1, 2, 5, 6]:  # dis_feats
                arrs[..., idx] = np.round(arrs[..., idx]).astype(np.int32)
            # =========================
            print(f"After type fix - arrs[..., 1] dtype: {arrs[..., 1].dtype}, unique: {np.unique(arrs[..., 1])[:10]}")
            meta = {'trips': arrs, 'lengths': valid_lens, 'trip_ids': trip_ids}
            ...
            trip_denormalizer = Denormalizer(self.stat, feat_cols=[0, 3, 4, 7], norm_type='minmax')
            ...
            max_trip_len = 107  # 固定最大长度
            for i in range(3):
                try:
                    trips, lengths, trip_ids = self.load_meta('trip', i)
    ...
    def merge_trip_npz(self):
        # 合并 trip_0.npz, trip_1.npz, trip_2.npz
        all_trips, all_lengths, all_trip_ids = [], [], []
        for i in range(3):
            meta_path = self.get_meta_path('trip', i)
            if os.path.exists(meta_path):
                loaded = np.load(meta_path, allow_pickle=True)
                all_trips.append(loaded['trips'])
                all_lengths.append(loaded['lengths'])
                all_trip_ids.append(loaded['trip_ids'])
            else:
                print(f"Warning: {meta_path} not found, skipping.")
        if not all_trips:
            raise ValueError("No trip_*.npz files found to merge.")
        trips = np.concatenate(all_trips, axis=0)
        lengths = np.concatenate(all_lengths, axis=0)
        trip_ids = np.concatenate(all_trip_ids, axis=0)
        merged_path = self.get_meta_path('trip', None)
        np.savez(merged_path, trips=trips, lengths=lengths, trip_ids=trip_ids)
        print(f"Merged {len(trip_ids)} trips into {merged_path}")
...
class Normalizer(nn.Module):
    ...
    def _norm_col(self, x_col, col_name):
        if self.norm_type == 'zscore':
            std = self.stat.loc['std', col_name]
            if std == 0:
                return np.zeros_like(x_col)
            x_col = (x_col - self.stat.loc['mean', col_name]) / std
        elif self.norm_type == 'minmax':
            denom = self.stat.loc['max', col_name] - self.stat.loc['min', col_name]
            if denom == 0:
                return np.zeros_like(x_col)
            x_col = (x_col - self.stat.loc['min', col_name]) / denom
            x_col = x_col * 2 - 1
        else:
